...
scan_commit_message() {
    ...
    if scan_text "${message}" "commit-message"; then
        ...
        printf '\n%s%sAction Required:%s Remove attribution markers before proceeding.\n\n' "${YELLOW}" "${BOLD}" "${NC}"
        return 1
    else
...
scan_codebase() {
    ...
    if [[ "${VIOLATIONS_FOUND}" -eq 1 ]]; then
        ...
        printf '\n%s%sAction Required:%s Remove attribution markers before proceeding.\n' "${YELLOW}" "${BOLD}" "${NC}"
        printf '%sReference: CLAUDE.md Section 1 — Protocol Zero%s\n\n' "${DIM}" "${NC}"
        return 1
    else
        log_pass "No attribution artifacts detected"
        printf '%sScanned %s files against %s patterns%s\n\n' "${DIM}" "${files_scanned}" "${#PROHIBITED_PATTERNS[@]}" "${NC}"
        return 0
    fi
}
